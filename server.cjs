...
const processAIMove = () => {
  ...
  setTimeout(() => {
    ...
    if (moveAction) {
        ...
        console.log("AI: 진짜 갈 곳이 없어요... (갇힘)");
        // 갇혔을 때 턴을 넘겨야 게임이 진행됨 (사실상 패배지만 일단 턴 넘김)
    }

    if (!newState.winner) {
        newState.turn = 1; // P1(사람)에게 턴 넘김
